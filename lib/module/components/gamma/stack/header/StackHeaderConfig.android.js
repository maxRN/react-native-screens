...
function StackHeaderConfig(props, forwardedRef) {
  ...
  const handleRendererResolved = event => {
    const {
      requestedRenderer,
      actualRenderer,
      fallbackReason
  ...
  const scrollFlagProps = resolveScrollFlags(filteredAndroidProps.type, filteredAndroidProps.renderer, {
    scrollFlagScroll,
    scrollFlagEnterAlways,
    scrollFlagEnterAlwaysCollapsed,
    scrollFlagExitUntilCollapsed,
...
function resolveScrollFlags(type, renderer, overrides) {
  const defaults = type === 'medium' && renderer === 'compose' ? COMPOSE_MEDIUM_SCROLL_FLAG_DEFAULTS : SCROLL_FLAG_DEFAULTS_BY_TYPE[type ?? 'small'];
  return {
...
const COMPOSE_MEDIUM_SCROLL_FLAG_DEFAULTS = {
  scrollFlagScroll: true,
